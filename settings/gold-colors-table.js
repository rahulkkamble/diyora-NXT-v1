...
function createTableRowForColor(data) {
    ...
    row.innerHTML = `
        ...
        <td class="align-middle border border-1">
        <p class="text-xs ps-2 fw-bold mb-0">${data.goldColor}</p>
        </td>
        <td class="align-middle border border-1">
            <p class="text-xs text-wrap fw-bold mb-0">${data.goldColorDescription}</p>
        </td>
...
var backupTableBody = document.querySelector('#gold-color-table tbody');
...
async function processDataForColor() {
    try {
        const dataArray = await getData();
        dataArray.forEach(dataItem => {
            var row = createTableRowForColor(dataItem);
            backupTableBody.appendChild(row);
        });
        // Initialize DataTables
        var dataTable = new DataTable(document.querySelector('#gold-color-table'), {
            "pageLength": 5,
            "language": {
